...
load_csv() {
    ...
    local base_table_name="${csv_file%.csv}"
    
    # For staging schema, append _raw to table names
    local table_name="$base_table_name"
    if [ "$DB_SCHEMA" = "staging" ]; then
        table_name="${base_table_name}_raw"
    fi
