...
BEGIN
    ...
    -- Use a simpler batching approach - create batches based on ID ranges
    -- First, truncate any existing batch data
    TRUNCATE TABLE staging.observation_migration_batches;
    
    -- Create batches with a simpler approach
    ...
    LOOP
        ...
        IF v_batch_size_actual > 0 THEN
            ...
            IF v_current_min_id IS NULL OR v_current_max_id = v_max_id THEN
                EXIT;
            END IF;
        ELSE
...
-- Verification Check
-- Records that failed to migrate (should be zero):
...
-- Data Samples
-- Sample records from measurements_raw:
