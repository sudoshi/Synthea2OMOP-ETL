#!/usr/bin/env python3
"""
fast_fix_observations.py - An optimized script to process remaining observations 
...
def process_measurement_batch_optimized(conn, batch, progress_tracker, step_name, 
    ...
    try:
        ...
        for row in batch:
            ...
            mapping_data.append((src_id,))
        
        with conn.cursor() as cur:
            # Create all mapping records in a single bulk operation
            execute_values(
                cur,
                """
                ...
                """,
                [(src_id, "nextval('staging.measurement_seq')") for src_id, in mapping_data],
                template="(%s, " + "%s" + ")",
                page_size=10000  # Process in larger pages
            )
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            execute_values(
                ...
                """,
                [(src_id, "nextval('staging.observation_seq')") for src_id, in mapping_data],
                template="(%s, " + "%s" + ")",
                page_size=10000  # Process in larger pages
            )
...
def resume_measurement_processing():
    ...
    try:
        ...
        # Add a trigram index to help with string operations
        try:
            with conn.cursor() as cur:
                cur.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm;")
                cur.execute(f"CREATE INDEX IF NOT EXISTS idx_{temp_table.replace('.', '_')}_value_trgm ON {temp_table} USING gin (value_as_string gin_trgm_ops);")
                conn.commit()
                logging.info("Added trigram index for better text search performance")
        except Exception as e:
            logging.warning(f"Could not create trigram index: {e}")
            conn.rollback()
        ...
        # Setup batch processing with a much larger batch size
        chunk_size = 100000  # Larger batch size for better throughput
